...
const manifest = {
  ...
  category: "tools",
  tags: ["测试套件", "全功能", "诊断", "HUD", "属性面板", "工具栏", "着色器"],
  configSchema: [
    {
      key: "showHudProbe",
    ...
    },
    {
      ...
      default: "#8b5cf6",
      options: [
        { label: "💜 极光紫 (#8b5cf6)", value: "#8b5cf6" },
        { label: "💚 荧光绿 (#10b981)", value: "#10b981" },
        { label: "💙 科技蓝 (#0ea5e9)", value: "#0ea5e9" },
        { label: "🧡 活力橙 (#f97316)", value: "#f97316" }
      ]
    }
  ],
  defaultConfig: {
...
const indexJs = `// OpenLVET 全能插件功能测试套件
module.exports = {
...
const readme = `# 全能插件功能测试套件 (Plugin Feature Tester v1.0.0)
...
// Save files
const outputDir = path.resolve("sample-plugins", "plugin-feature-tester");
...
fs.writeFileSync(path.resolve("sample-plugins", "plugin-feature-tester.zip"), Buffer.from(zipData));
fs.writeFileSync(path.resolve("plugin-feature-tester.zip"), Buffer.from(zipData));

console.log("Successfully built plugin-feature-tester.zip!");
