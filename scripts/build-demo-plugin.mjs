  ...
  configSchema: [
    ...
    },
    {
      ...
      options: [
        ...
        { label: "霓虹绿", value: "#4ade80" },
        { label: "活力橙", value: "#fb923c" },
...
const readme = `# 实时时间码与安全框 HUD 插件 (v1.0.0)
...
// Save unpackaged files to sample-plugins/demo-watermark-hud/
const outputDir = path.resolve("sample-plugins", "demo-watermark-hud");
...
// Save ZIP file in sample-plugins/ and project root for convenience
fs.writeFileSync(path.resolve("sample-plugins", "demo-watermark-hud.zip"), Buffer.from(zipData));
fs.writeFileSync(path.resolve("demo-watermark-hud.zip"), Buffer.from(zipData));

console.log("Successfully created demo-watermark-hud.zip at workspace root and sample-plugins/");
