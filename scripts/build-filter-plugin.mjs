  ...
  configSchema: [
    {
      ...
      default: "teal-orange",
      options: [
        { label: "🎬 好莱坞青橙 (Teal & Orange)", value: "teal-orange" },
        { label: "🎞️ 复古胶片颗粒 (Vintage Film Grain)", value: "vintage-film" },
        { label: "🌸 日系唯美柔光 (Anime Pastel Dream)", value: "pastel-dream" }
      ]
    },
    {
...
const indexJs = `// OpenLVET 大师级电影调色与复古胶片滤镜插件
module.exports = {
...
const readme = `# 大师级电影调色与复古胶片滤镜 (Cinematic & Film FX v1.0.0)
...
// Save files
const outputDir = path.resolve("sample-plugins", "cinematic-film-filters");
...
fs.writeFileSync(path.resolve("sample-plugins", "cinematic-film-filters.zip"), Buffer.from(zipData));
fs.writeFileSync(path.resolve("cinematic-film-filters.zip"), Buffer.from(zipData));

console.log("Successfully built cinematic-film-filters.zip!");
