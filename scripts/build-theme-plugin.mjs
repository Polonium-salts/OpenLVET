  ...
  configSchema: [
    {
      ...
      default: "cyberpunk",
      options: [
        { label: "🌌 赛博霓虹 (Cyberpunk Purple)", value: "cyberpunk" },
        { label: "🍃 极光翡翠 (Emerald Aurora)", value: "emerald" },
        { label: "🌅 日落暮光 (Sunset Amber)", value: "sunset" },
        { label: "🌸 樱花绯粉 (Sakura Rose)", value: "sakura" },
        { label: "🌊 深海湛蓝 (Ocean Cobalt)", value: "ocean" },
        { label: "⚡ 极客冷灰 (Monochrome)", value: "monochrome" },
        { label: "🎨 自定义主色 (Custom Hex)", value: "custom" }
      ]
    },
    {
      key: "customPrimary",
...
const indexJs = `// OpenLVET 全局主题定制大师插件
module.exports = {
...
const readme = `# 全局主题定制大师 (Theme Master v1.0.0)

OpenLVET 全局色彩定制与主题切换插件。
`;

// Save files
const outputDir = path.resolve("sample-plugins", "global-theme-master");
...
fs.writeFileSync(path.resolve("sample-plugins", "global-theme-master.zip"), Buffer.from(zipData));
fs.writeFileSync(path.resolve("global-theme-master.zip"), Buffer.from(zipData));

console.log("Successfully built global-theme-master.zip!");
