...
export function Captions() {
	...
	const handleProgress = (progress: TranscriptionProgress) => {
		if (progress.status === "loading-model") {
			dispatch({
				type: "update_step",
				step: `正在加载语音模型 ${Math.round(progress.progress)}%`,
			});
		} else if (progress.status === "transcribing") {
			dispatch({ type: "update_step", step: "正在识别转录字幕..." });
		}
	};
	...
	const handleGenerateTranscript = async () => {
		dispatch({ type: "start", step: "正在提取音轨..." });
		try {
			...
			dispatch({ type: "update_step", step: "正在预处理音频..." });
			...
			if (!insertCaptions({ captions: captionChunks })) {
				dispatch({ type: "fail", error: "未生成任何有效字幕" });
				return;
			}
	...
	const handleImportFile = async ({ file }: { file: File }) => {
		...
		try {
			...
			if (!insertCaptions({ captions: result.captions })) {
				dispatch({ type: "fail", error: "导入字幕失败" });
				return;
			}
			...
			if (result.skippedCueCount > 0) {
				nextWarnings.unshift(
					`成功导入 ${result.captions.length} 条字幕，跳过了 ${result.skippedCueCount} 条格式异常的片段。`,
				);
			}
